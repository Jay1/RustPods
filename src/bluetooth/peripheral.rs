...
use btleplug::api::{
    BDAddr, Characteristic, Peripheral as _, WriteType, 
    CharPropFlags
};
use uuid::Uuid;
...
impl BlePeripheral {
    ...
    pub async fn is_connected(&self) -> Result<bool, BluetoothError> {
        ...
        match self.peripheral.is_connected().await {
            ...
            Err(e) => {
                ...
                Err(BluetoothError::ApiError(format!("Failed to check connection status: {}", e)))
            }
        }
    }
    ...
    pub async fn connect(&mut self) -> Result<(), BluetoothError> {
        ...
        while attempt < max_attempts {
            attempt += 1;
            debug!("{}Attempting to connect (attempt {}/{})", ctx, attempt, max_attempts);
            ...
            match self.peripheral.connect().await {
                ...
                Err(e) => {
                    if attempt < max_attempts {
                        warn!("{}Connection attempt {} failed: {}. Retrying...", ctx, attempt, e);
                        // Wait before retry with increasing delay
                        tokio::time::sleep(Duration::from_millis(500 * attempt as u64)).await;
                        continue;
                    } else {
                        error!("{}Failed to connect after {} attempts: {}", ctx, max_attempts, e);
        ...
        Err(BluetoothError::ConnectionFailed("Failed to connect".to_string()))
    }
    ...
    pub async fn disconnect(&mut self) -> Result<(), BluetoothError> {
        ...
        match self.peripheral.disconnect().await {
            ...
            Err(e) => {
                ...
                Err(BluetoothError::DeviceDisconnected(
    ...
    pub async fn read_characteristic(&self, uuid: Uuid) -> Result<Vec<u8>, BluetoothError> {
        ...
        if !self.is_connected {
            return Err(BluetoothError::ConnectionFailed("Not connected".to_string()));
        }
        ...
        let characteristic = characteristics.iter()
            .find(|c| c.uuid == uuid)
            .ok_or_else(|| {
                let err = BluetoothError::InvalidData(format!("Characteristic not found: {}", uuid));
        ...
        match self.peripheral.read(characteristic).await {
            Ok(value) => {
                debug!("{}Successfully read characteristic, got {} bytes", ctx, value.len());
                Ok(value)
            },
            Err(e) => {
                error!("{}Failed to read characteristic: {}", ctx, e);
                Err(BluetoothError::ApiError(format!("Failed to read characteristic: {}", e)))
            }
        }
    }
    
    /// Write a value to a characteristic
    pub async fn write_characteristic(&self, uuid: Uuid, data: &[u8], write_type: WriteType) -> Result<(), BluetoothError> {
        ...
        match self.peripheral.write(characteristic, data, write_type).await {
            Ok(()) => {
                debug!("{}Successfully wrote {} bytes to characteristic", ctx, data.len());
                Ok(())
            },
            Err(e) => {
                error!("{}Failed to write characteristic: {}", ctx, e);
                Err(BluetoothError::ApiError(format!("Failed to write characteristic: {}", e)))
            }
        }
    }
    
    /// Subscribe to characteristic notifications
    pub async fn subscribe(&mut self, uuid: Uuid, handler: NotificationHandler) -> Result<(), BluetoothError> {
        ...
        if !characteristic.properties.contains(CharPropFlags::NOTIFY) {
            ...
            return Err(BluetoothError::InvalidData(
                format!("Characteristic {} doesn't support notifications", uuid)
            ));
        }
        ...
        match self.peripheral.subscribe(characteristic).await {
            Ok(()) => {
                ...
                // Store the handler
                {
                    let mut handlers = self.notification_handlers.lock()
                        .map_err(|e| {
                            error!("{}Failed to lock notification handlers: {}", ctx, e);
                            BluetoothError::Other(format!("Failed to lock notification handlers: {}", e))
                        })?;
            ...
            Err(e) => {
                ...
                Err(BluetoothError::ApiError(format!("Failed to subscribe to notifications: {}", e)))
            }
        }
    }
    ...
    pub async fn unsubscribe(&self, uuid: Uuid) -> Result<(), BluetoothError> {
        ...
        match self.peripheral.unsubscribe(characteristic).await {
            ...
            Err(e) => {
                ...
                Err(BluetoothError::ApiError(format!("Failed to unsubscribe from notifications: {}", e)))
            }
        }
    }
    ...
    pub async fn discover_characteristics(&self) -> Result<Vec<Characteristic>, BluetoothError> {
        ...
        match self.peripheral.discover_services().await {
            ...
            Err(e) => {
                ...
                Err(BluetoothError::ApiError(format!("Failed to discover services: {}", e)))
            }
        }
    }
    ...
    pub async fn connect_service(&mut self, service_uuid: Uuid) -> Result<(), BluetoothError> {
        ...
        let _service = services.iter()
            .find(|s| s.uuid == service_uuid)
            .ok_or_else(|| {
                let err = BluetoothError::InvalidData(format!("Service not found: {}", service_uuid));
