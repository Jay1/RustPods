...
use crate::airpods::{AirPodsBattery, AirPodsChargingState, DetectedAirPods};
...
impl BatteryMonitor {
                            ...
                            if options.use_smoothing {
                                ...
                                if monitor_guard.is_valid_battery(&smoothed) {
                                    ...
                                    if options.adaptive_polling {
                                        let new_interval =
                                            monitor_guard.calculate_adaptive_interval(&smoothed);
                                        if new_interval != monitor_guard.current_interval {
                                            ...
                                            if (new_interval.as_secs_f32()
                                                / monitor_guard.current_interval.as_secs_f32()
                                                - 1.0)
                                                .abs()
                                                > 0.2
                                            {
                                    ...
                                    if let Some(last_reading) = &monitor_guard.last_valid_reading {
                                        ...
                                        if monitor_guard
                                            .has_significant_change(last_reading, &smoothed)
                                        {
                                            ...
                                            if options.notify_low_battery {
                                                _alert_result =
                                                    monitor_guard.check_low_battery(&smoothed);
                                ...
                                if monitor_guard.is_valid_battery(battery) {
                                    ...
                                    if options.adaptive_polling {
                                        let new_interval =
                                            monitor_guard.calculate_adaptive_interval(battery);
                                        if new_interval != monitor_guard.current_interval {
                                            // Update the interval
                                            monitor_guard.current_interval = new_interval;
                                    ...
                                    if let Some(last_reading) = &monitor_guard.last_valid_reading {
                                        // Only report significant changes
                                        if monitor_guard
                                            .has_significant_change(last_reading, battery)
                                        {
                                            // Create battery status
                                            let status = AirPodsBatteryStatus {
    ...
    fn check_low_battery(&mut self, battery: &AirPodsBattery) -> Option<BatteryAlert> {
        ...
        if let Some(case) = battery.case {
            ...
            let is_charging = matches!(&battery.charging, Some(AirPodsChargingState::CaseCharging));
