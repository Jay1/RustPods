...
pub use examples::{airpods_filtering, discover_adapters, interval_scanning, scan_with_adapter};
...
pub fn bluetooth_error_with_recovery(
    ...
    match error_type {
        ...
        BluetoothError::AdapterRefreshFailed { error, .. } => {
        ...
        BluetoothError::AdapterNotAvailable { reason, .. } => {
            BluetoothError::AdapterNotAvailable { reason, recovery }
        }
        BluetoothError::AdapterScanFailed { error, .. } => {
...
pub fn convert_btleplug_error(
    ...
    match error {
        BtlePlugError::PermissionDenied => {
            BluetoothError::PermissionDenied(format!("Permission denied during {}", operation))
        }
        BtlePlugError::DeviceNotFound => {
            BluetoothError::DeviceNotFound(format!("Device not found during {}", operation))
        }
        BtlePlugError::NotConnected => {
            BluetoothError::ConnectionFailed(format!("Device not connected during {}", operation))
        }
        BtlePlugError::InvalidBDAddr(_) => {
            BluetoothError::InvalidData("Invalid Bluetooth address format".to_string())
        }
        BtlePlugError::Uuid(_) => BluetoothError::InvalidData("Invalid UUID format".to_string()),
        BtlePlugError::NotSupported(_) => {
            BluetoothError::Other(format!("Operation not supported: {}", operation))
        }
        BtlePlugError::Other(msg) => BluetoothError::Other(msg.to_string()),
        _ => BluetoothError::Other(format!("Unknown Bluetooth error during {}", operation)),
    }
}
