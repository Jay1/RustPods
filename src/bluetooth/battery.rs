...
pub async fn extract_battery_data(
    peripheral: &Peripheral,
) -> Result<AirPodsBattery, BluetoothError> {
    let properties = match peripheral.properties().await {
        Ok(Some(props)) => props,
        Ok(None) => {
            return Err(BluetoothError::Other(
                "No device properties found".to_string(),
            ))
        }
    ...
    let apple_data = match properties.manufacturer_data.get(&APPLE_COMPANY_ID) {
        ...
        None => {
            return Err(BluetoothError::Other(
                "No Apple manufacturer data found".to_string(),
            ))
        }
    };
    ...
    match parse_airpods_data(apple_data) {
        ...
        Err(err) => Err(BluetoothError::Other(format!(
            "Failed to parse AirPods battery data: {}",
            err
        ))),
    }
}
...
mod tests {
    ...
    fn test_battery_status_display() {
        ...
        assert_eq!(
            status.to_string(),
            "Left: 80%, Right: 75%, Case: 90%, Charging: Unknown"
        );
        ...
        assert_eq!(
            status.to_string(),
            "Left: 80%, Right: Unknown%, Case: 90%, Charging: Unknown"
        );
        ...
        assert_eq!(
            status.to_string(),
            "Left: Unknown%, Right: Unknown%, Case: Unknown%, Charging: Unknown"
        );
    }
    ...
    fn test_update_battery_status() {
        ...
        assert_eq!(
            status.battery.right, None,
            "Right earbud value should be None"
        );
