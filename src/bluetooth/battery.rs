...
impl AirPodsBatteryStatus {
    ...
    pub fn is_stale(&self, duration: chrono::Duration) -> bool {
        chrono::Utc::now().signed_duration_since(self.last_updated) > duration
    }
}
...
mod tests {
    ...
    fn test_battery_status_display() {
        ...
        assert_eq!(status.to_string(), "Left: 80%, Right: 75%, Case: 90%");
        ...
        let status = AirPodsBatteryStatus::new(battery);
        assert_eq!(status.to_string(), "Left: 80%, Right: Unknown%, Case: 90%");
        
        // Test with all values missing
        let status = AirPodsBatteryStatus::default();
        assert_eq!(status.to_string(), "Left: Unknown%, Right: Unknown%, Case: Unknown%");
    }
    ...
    fn test_is_stale() {
        ...
        assert!(!status.is_stale(chrono::Duration::seconds(90)));
        
        // Test with current timestamp
        let current_status = AirPodsBatteryStatus::new(AirPodsBattery::default());
        assert!(!current_status.is_stale(chrono::Duration::seconds(5)));
        
        // Test with exactly matching duration
        // Create a timestamp exactly 30 seconds in the past
        let exact_past_time = chrono::Utc::now() - chrono::Duration::seconds(30);
        ...
        assert!(exact_status.is_stale(chrono::Duration::seconds(29)));
    ...
    fn test_default_battery_status() {
        ...
        assert!(!default_status.battery.charging.left);
        assert!(!default_status.battery.charging.right);
        assert!(!default_status.battery.charging.case);
    ...
    fn test_charging_status_changes() {
        ...
        let charging_battery = create_test_battery_with_charging(
            Some(50), Some(60), Some(70), true, true, true
        );
        ...
        let non_charging_battery = create_test_battery_with_charging(
            Some(55), Some(65), Some(75), false, false, false
        );
        ...
        let mixed_charging_battery = create_test_battery_with_charging(
            Some(60), Some(70), Some(80), false, false, true
        );
