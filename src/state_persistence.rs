...
impl StatePersistenceManager {
    ...
    pub fn new(state_manager: Arc<StateManager>) -> Self {
        let state_path =
            Self::get_state_file_path().unwrap_or_else(|_| PathBuf::from("rustpods_state.json"));
    ...
    pub fn apply_state(&self, state: PersistentState) -> Result<()> {
        ...
        let action =
            crate::ui::state_manager::Action::SetAdvancedDisplayMode(state.advanced_display_mode);
        ...
        if let Some(device_address) = state.selected_device {
            ...
            if state.connection_state == "Connected" || state.connection_state == "Reconnecting" {
                ...
                let action =
                    crate::ui::state_manager::Action::RestorePreviousConnection(device_address);
                self.state_manager.dispatch(action);
            }
        }
