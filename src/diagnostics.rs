...
impl DiagnosticsManager {
    ...
    pub fn new(config: Arc<AppConfig>, error_manager: Arc<Mutex<ErrorManager>>) -> Self {
        Self {
            config,
            error_manager,
            last_result: None,
    ...
    pub async fn run_diagnostics(&mut self) -> io::Result<DiagnosticResult> {
        ...
        if let Err(e) = self
            .check_bluetooth(&mut issues, &mut recommendations, &mut raw_data)
            .await
        {
        ...
        has_critical_issues =
            has_critical_issues || issues.iter().any(|i| i.severity == IssueSeverity::Critical);
    ...
    fn collect_system_information(&self, raw_data: &mut HashMap<String, String>) -> io::Result<()> {
        ...
        raw_data.insert(
            "log_level".to_string(),
            format!("{:?}", self.config.system.log_level),
        );
        raw_data.insert(
            "auto_scan".to_string(),
            self.config.bluetooth.auto_scan_on_startup.to_string(),
        );
        raw_data.insert(
            "start_minimized".to_string(),
            self.config.ui.start_minimized.to_string(),
        );
        ...
        if let Ok(error_manager) = self.error_manager.lock() {
            ...
            if bluetooth_errors > 5 {
                issues.push(DiagnosticIssue {
                    ...
                    description: format!(
                        "Detected {} Bluetooth errors, which may indicate compatibility issues",
                        bluetooth_errors
                    ),
                    solutions: vec![
                        "Ensure your Bluetooth adapter is compatible (Bluetooth 4.0+ recommended)"
                            .to_string(),
                        "Update Bluetooth drivers".to_string(),
                        "Try disabling other Bluetooth applications".to_string(),
                    ],
                    severity: if bluetooth_errors > 20 {
                        IssueSeverity::Major
                    } else {
                        IssueSeverity::Minor
                    },
            ...
            if critical_errors > 0 {
                issues.push(DiagnosticIssue {
                    ...
                    description: format!(
                        "Detected {} critical errors that may prevent core functionality",
                        critical_errors
                    ),
                    solutions: vec![
    ...
    async fn check_bluetooth(
        ...
        match adapter_result {
            Ok(adapter) => {
                ...
                raw_data.insert(
                    "bluetooth_scanning_supported".to_string(),
                    supports_scanning.to_string(),
                );
                ...
                recommendations
                    .push("Keep Bluetooth enabled for automatic detection of AirPods".to_string());
    ...
    fn check_configuration(
        ...
        if let Err(e) = self.config.validate() {
            ...
            recommendations
                .push("Reset to default configuration to resolve validation errors".to_string());
        }
        ...
        if self.config.bluetooth.scan_duration.as_secs() < 3 {
            ...
            recommendations
                .push("Increase Bluetooth scan duration to improve device detection".to_string());
        }
                    ...
                    match std::fs::write(&test_path, "test") {
                        ...
                        Err(e) => {
                            ...
                            issues.push(DiagnosticIssue {
                                ...
                                description: format!(
                                    "Insufficient permissions to write to config directory: {}",
                                    e
                                ),
                                solutions: vec![
    ...
    fn check_hardware_compatibility(
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            raw_data.insert(
                "bluetooth_version_check".to_string(),
                "simulated".to_string(),
            );
        }
    ...
    pub async fn auto_repair(
        &self,
    ) -> Result<AutoRepairResult, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let repairable_issues: Vec<&DiagnosticIssue> = diagnostic
            .issues
            .iter()
            ...
            match issue.category {
                IssueCategory::Configuration => {
                    ...
                    if let Err(e) = self.repair_configuration_issue(issue).await {
                        ...
                        result
                            .failed_repairs
                            .push((issue.title.clone(), e.to_string()));
                    } else {
                ...
                _ => {
                    log::warn!(
                        "No repair implementation for issue category: {:?}",
                        issue.category
                    );
    ...
    async fn repair_configuration_issue(&self, issue: &DiagnosticIssue) -> Result<(), String> {
        ...
        Err(format!(
            "No repair implementation for issue: {}",
            issue.title
        ))
    }
}
...
pub fn create_troubleshooting_guide(result: &DiagnosticResult) -> Vec<TroubleshootingStep> {
    ...
    let critical_issues: Vec<&DiagnosticIssue> = result
        .issues
        .iter()
    ...
    let major_issues: Vec<&DiagnosticIssue> = result
        .issues
        .iter()
    ...
    if result
        .issues
        .iter()
        .any(|i| i.category == IssueCategory::Bluetooth)
    {
        steps.push(TroubleshootingStep {
