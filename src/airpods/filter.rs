...
use super::{identify_airpods_type, AirPodsType, Result};
                ...
                if let Some(model_types) = &self.model_types {
                    if let Some(data) = device.manufacturer_data.get(&APPLE_COMPANY_ID) {
                        if let Ok(device_type) = identify_airpods_type(&device.name, data) {
                            if !model_types.contains(&device_type)
...
mod tests {
    ...
    fn test_filter_by_rssi() {
        ...
        let strong_device = create_mock_device(
            Some("AirPods"),
            Some(-60),
            Some(vec![0x07, 0x19, 0x01, 0x02]),
            true,
        );
        let weak_device = create_mock_device(
            Some("AirPods"),
            Some(-80),
            Some(vec![0x07, 0x19, 0x01, 0x02]),
            true,
    ...
    fn test_filter_function() {
        ...
        let weak_pro = create_mock_device(
            Some("AirPods Pro"),
            Some(-80),
            Some(vec![0x0E, 0x19, 0x01, 0x02]),
            true,
        );
        let regular = create_mock_device(
            Some("AirPods"),
            Some(-60),
            Some(vec![0x07, 0x19, 0x01, 0x02]),
            true,
