...
impl AppStateController {
    ...
    pub async fn initialize(&mut self) -> Result<(), String> {
        ...
        let lifecycle_manager =
            LifecycleManager::new(Arc::clone(&self.state_manager), self.ui_sender.clone())
        ...
        if settings.bluetooth.auto_reconnect {
                ...
                .clone()
            {
                ...
                tokio::spawn(async move {
                    ...
                    // Try to reconnect
                    if let Err(e) = reconnect_controller
                        .restore_previous_connection(last_device_clone)
                        .await
                    {
    ...
    pub async fn restore_previous_connection(
        &mut self,
        device_address: String,
    ) -> Result<(), String> {
        info!(
            "Attempting to restore previous connection to: {}",
            device_address
        );
