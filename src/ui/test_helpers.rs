...
impl MockSystemTray {
    ...
    fn check_low_battery(&mut self, battery: &AirPodsBatteryStatus) {
        ...
        if let Some(left) = battery.battery.left {
            if left <= self.low_battery_threshold {
                self.notifications
                    .push(format!("Low Battery Warning: Left AirPod at {}%", left));
            }
        }
        ...
        if let Some(right) = battery.battery.right {
            if right <= self.low_battery_threshold {
                self.notifications
                    .push(format!("Low Battery Warning: Right AirPod at {}%", right));
            }
        }
        ...
        if let Some(case) = battery.battery.case {
            if case <= self.low_battery_threshold {
                self.notifications
                    .push(format!("Low Battery Warning: Case at {}%", case));
            }
        }
    }
    ...
    pub fn update_connection(&mut self, connected: bool) {
        ...
        self.actions
            .push(format!("update_connection: {}", connected));
    }
...
impl TestForm {
    ...
    pub fn validate_field(&mut self, name: &str) {
        ...
        if name == "required_field" && value.is_empty() {
            self.errors
                .insert(name.to_string(), "Field is required".to_string());
        } else if name == "number_field" {
            if value.parse::<i32>().is_err() {
                self.errors
                    .insert(name.to_string(), "Must be a number".to_string());
            } else if let Ok(num) = value.parse::<i32>() {
                if !(0..=100).contains(&num) {
                    self.errors.insert(
                        name.to_string(),
                        "Number must be between 0 and 100".to_string(),
                    );
                } else {
