...
impl RealTimeBatteryDisplay {
    ...
    pub fn update(&mut self, battery_status: Option<AirPodsBatteryStatus>) {
        ...
        if let Some(current_status) = &self.battery_status {
            ...
            self.previous_levels = Some((battery.left, battery.right, battery.case));
        }
    ...
    fn calculate_pulse_effect(&self) -> f32 {
        let pulse = (1.0
            + (self.animation_progress * CHARGING_PULSE_SPEED * std::f32::consts::PI * 2.0).sin())
            * 0.5;
    ...
    fn create_battery_bar(
        ...
        let progress = progress_bar(0.0..=1.0, level_f32).height(18.0);

        let charging_icon = if is_charging { text("⚡") } else { text("") };
    ...
    fn create_status_summary(&self) -> Element<'static, Message, iced::Renderer<Theme>> {
        if let Some(status) = &self.battery_status {
            ...
            let time_text = if let Some(minutes) = self.calculate_time_remaining() {
                format!(
                    " • Approx. {} remaining",
                    self.format_time_remaining(minutes)
                )
            } else {
                "".to_string()
            };
    ...
    pub fn create_static_view(
        battery_status: AirPodsBatteryStatus,
    ) -> Element<'static, Message, iced::Renderer<Theme>> {
        let display = Self::new(Some(battery_status));

        let mut content = column![].spacing(15).width(Length::Fill);
        ...
        if let Some(status) = &display.battery_status {
            ...
            let (prev_left, prev_right, prev_case) =
                display.previous_levels.unwrap_or((None, None, None));

            // Add battery bars
            content = content.push(
                display.create_battery_bar(
                    "Left",
                    status.battery.left,
                    prev_left,
                    status
...
impl UiComponent for RealTimeBatteryDisplay {
    fn view(&self) -> Element<'static, Message, iced::Renderer<Theme>> {
        ...
        if let Some(status) = &self.battery_status {
            // Get previous levels for animation
            let (prev_left, prev_right, prev_case) =
                self.previous_levels.unwrap_or((None, None, None));

            // Add battery bars
            content = content.push(
                self.create_battery_bar(
                    "Left",
                    status.battery.left,
                    prev_left,
                    status
