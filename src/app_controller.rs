...
use crate::bluetooth::{BleEvent, BleScanner, EventBroker, EventFilter, ScanConfig};
        ...
        tokio::spawn(async move {
            while let Some(event) = rx_airpods.recv().await {
                if let BleEvent::AirPodsDetected(airpods) = event {
                    ...
                    ui_sender_clone
                        .send(Message::AirPodsConnected(airpods.clone()))
                        .ok();
            ...
            while let Some(event) = rx_device.recv().await {
                match event {
                    ...
                    BleEvent::DeviceLost(address) => {
                        ...
                        let _ =
                            ui_sender.send(Message::ShowToast(format!("Device lost: {}", address)));
                    }
    ...
    pub fn shutdown(&mut self) -> Result<(), String> {
        ...
        self.run_async(async move {
            ...
            if is_scanning {
                ...
                let mut scanner_option: Option<BleScanner> =
                    std::mem::take(&mut *scanner_arc.lock().unwrap());
        ...
        loop {
            ...
            if let Some(_airpods) = device {
                ...
                let battery_level = crate::airpods::AirPodsBattery {
                    left: Some(
                        (80 + (tokio::time::Instant::now().elapsed().as_secs() % 20) as u8)
                            .min(100),
                    ),
                    right: Some(
                        (75 + (tokio::time::Instant::now().elapsed().as_secs() % 20) as u8)
                            .min(100),
                    ),
                    case: Some(
                        (90 + (tokio::time::Instant::now().elapsed().as_secs() % 10) as u8)
                            .min(100),
                    ),
                    charging: Some(
                        if (tokio::time::Instant::now().elapsed().as_secs() % 60) < 30 {
