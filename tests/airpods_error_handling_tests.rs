...
fn test_graceful_degradation() {
    ...
    assert!(
        detected.battery.is_none(),
        "Battery should be None with missing data"
    );
}
...
fn test_conversion_to_rustpods_error() {
    ...
    match rustpods_error {
        RustPodsError::AirPods(msg) => {
            assert!(
                format!("{}", msg).contains("parse error"),
                "Error message should contain 'parse error'"
            );
        }
        _ => panic!(
            "Expected RustPodsError::AirPods variant but got {:?}",
            rustpods_error
        ),
    }
}
...
fn test_error_manager_integration() {
    ...
    error_manager.record_error_with_context(airpods_error.into(), context, RecoveryAction::Retry);
