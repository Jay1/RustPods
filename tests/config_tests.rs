...
use rustpods::config::{
    AppConfig, BluetoothConfig, ConfigError, ConfigManager, LogLevel, SystemConfig, Theme, UiConfig,
};
use std::fs;
...
fn test_config_save_load() {
    ...
    let loaded_config =
        AppConfig::load_from_path(&config_path).expect("Failed to load configuration");
...
fn test_config_validation() {
    ...
    match result {
        Err(ConfigError::ValidationFailed(field, _)) => {
            assert!(
                field == "bluetooth.scan_duration" || field == "ui.low_battery_threshold",
                "Validation should fail on scan_duration or low_battery_threshold, got: {}",
                field
            );
        }
...
fn test_bluetooth_config_defaults() {
    ...
    assert!(
        config.scan_duration > Duration::from_secs(0)
            && config.scan_duration <= Duration::from_secs(60),
    ...
    assert!(
        config.min_rssi.is_none() || config.min_rssi.unwrap() < 0,
        "Default min RSSI should be negative (typical BLE values are negative) or None"
    );

    assert!(
        config.battery_refresh_interval > Duration::from_secs(0),
...
fn test_ui_config_defaults() {
    ...
    assert!(
        config.low_battery_threshold > 0 && config.low_battery_threshold <= 100,
        "Default battery threshold should be within 1-100%"
    );

    // The default theme should be a valid theme name
    assert!(
        matches!(config.theme, Theme::System | Theme::Light | Theme::Dark),
...
fn test_config_serialization() {
    ...
    assert_eq!(
        deserialized.bluetooth.auto_scan_on_startup,
        config.bluetooth.auto_scan_on_startup
    );
    assert_eq!(
    ...
    assert_eq!(
        deserialized.ui.show_notifications,
        config.ui.show_notifications
    );
    assert_eq!(deserialized.ui.theme, config.ui.theme);
    assert_eq!(
        deserialized.ui.low_battery_threshold,
        config.ui.low_battery_threshold
    );
    assert_eq!(
        deserialized.system.launch_at_startup,
        config.system.launch_at_startup
    );
    assert_eq!(deserialized.system.log_level, config.system.log_level);
    assert_eq!(
        deserialized.system.enable_telemetry,
        config.system.enable_telemetry
    );
}
...
fn test_invalid_config_files() {
    ...
    fs::write(
        &incomplete_json_path,
        r#"{"bluetooth": {"scan_duration": 5}}"#,
    )
    ...
    assert!(
        result.is_ok(),
        "Loading incomplete JSON should succeed with defaults"
    );
...
fn test_config_manager_updates() {
    ...
    assert!(
        config_path.exists(),
        "Config file should exist after update with auto-save"
    );
    ...
    assert!(
        config_after_invalid.ui.low_battery_threshold <= 100,
...
fn test_validation_edge_cases() {
    ...
    assert!(
        ...
        "Very short scan duration should fail validation"
    );
    ...
    assert!(
        ...
        "Very long scan duration is allowed by current validation"
    );
    ...
    assert!(
        config.ui.validate().is_ok(),
        "100% battery threshold should be valid"
    );

    config.ui.low_battery_threshold = 101; // Just above upper bound
    assert!(
        config.ui.validate().is_err(),
        "Battery threshold > 100% should fail validation"
    );
}
