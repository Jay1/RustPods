...
fn test_detect_airpods_regular() {
    ...
    assert_eq!(airpods.battery.left, Some(80), "Left battery should be 80%");
    assert_eq!(airpods.battery.right, Some(70), "Right battery should be 70%");
    assert_eq!(airpods.battery.case, Some(60), "Case battery should be 60%");
    assert!(!airpods.battery.charging.left, "Left should not be charging");
    assert!(!airpods.battery.charging.right, "Right should not be charging");
    assert!(airpods.battery.charging.case, "Case should be charging");
}
...
fn test_detect_airpods_pro() {
    ...
    assert_eq!(airpods.battery.left, Some(90), "Left battery should be 90%");
    assert_eq!(airpods.battery.right, Some(90), "Right battery should be 90%");
    assert_eq!(airpods.battery.case, Some(50), "Case battery should be 50%");
    assert!(airpods.battery.charging.left, "Left should be charging");
    assert!(airpods.battery.charging.right, "Right should be charging");
    assert!(!airpods.battery.charging.case, "Case should not be charging");
}
...
fn test_detect_airpods_pro_2() {
    ...
    assert_eq!(airpods.battery.left, Some(100), "Left battery should be 100%");
    assert_eq!(airpods.battery.right, Some(100), "Right battery should be 100%");
    assert_eq!(airpods.battery.case, Some(100), "Case battery should be 100%");
    assert!(!airpods.battery.charging.left, "Left should not be charging");
    assert!(!airpods.battery.charging.right, "Right should not be charging");
    assert!(airpods.battery.charging.case, "Case should be charging");
}
...
fn test_detect_airpods_max() {
    ...
    assert_eq!(airpods.battery.left, Some(60), "Left battery should be 60%");
    assert_eq!(airpods.battery.right, Some(60), "Right battery should be 60%");
    assert_eq!(airpods.battery.case, Some(0), "Case battery should be 0%");
    assert!(!airpods.battery.charging.left, "Left should not be charging");
    assert!(!airpods.battery.charging.right, "Right should not be charging");
    assert!(!airpods.battery.charging.case, "Case should not be charging");
}
...
fn test_detect_airpods_3() {
    ...
    assert_eq!(airpods.battery.left, Some(80), "Left battery should be 80%");
    assert_eq!(airpods.battery.right, Some(80), "Right battery should be 80%");
    assert_eq!(airpods.battery.case, Some(70), "Case battery should be 70%");
    assert!(!airpods.battery.charging.left, "Left should not be charging");
    assert!(!airpods.battery.charging.right, "Right should not be charging");
    assert!(!airpods.battery.charging.case, "Case should not be charging");
}
...
fn test_detect_airpods_empty_battery() {
    ...
    let airpods = result.unwrap();
    assert!(matches!(airpods.device_type, AirPodsType::AirPods1 | AirPodsType::AirPods2));
    assert_eq!(airpods.battery.left, None, "Left battery should be None");
    assert_eq!(airpods.battery.right, None, "Right battery should be None");
    assert_eq!(airpods.battery.case, None, "Case battery should be None");
}
...
fn test_identify_airpods_type() {
    ...
    assert_eq!(
        identify_airpods_type(AIRPODS_1_2_PREFIX), 
        AirPodsType::AirPods1,
        "Should identify AirPods 1/2"
    );
    
    assert_eq!(
        identify_airpods_type(AIRPODS_PRO_PREFIX), 
        AirPodsType::AirPodsPro,
        "Should identify AirPods Pro"
    );
    
    assert_eq!(
        identify_airpods_type(AIRPODS_PRO_2_PREFIX), 
        AirPodsType::AirPodsPro2,
        "Should identify AirPods Pro 2"
    );
    
    assert_eq!(
        identify_airpods_type(AIRPODS_3_PREFIX), 
        AirPodsType::AirPods3,
        "Should identify AirPods 3"
    );
    
    assert_eq!(
        identify_airpods_type(AIRPODS_MAX_PREFIX), 
    ...
    // Test unknown prefix
    assert_eq!(
        identify_airpods_type(&[0x00, 0x00]), 
        AirPodsType::Unknown,
...
fn test_airpods_default_trait_implementations() {
    ...
    assert_eq!(default_airpods.battery.left, None);
    assert_eq!(default_airpods.battery.right, None);
    assert_eq!(default_airpods.battery.case, None);
    assert!(!default_airpods.battery.charging.left);
    assert!(!default_airpods.battery.charging.right);
    assert!(!default_airpods.battery.charging.case);
    ...
    assert!(!default_battery.charging.left);
    assert!(!default_battery.charging.right);
    assert!(!default_battery.charging.case);
} 