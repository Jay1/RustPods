...
use rustpods::ui::test_helpers::{
    create_test_battery, create_test_config, create_test_state_manager, MockSystemTray,
};
...
fn test_system_tray_api() {
    ...
    assert!(
        result.is_ok(),
        "Should be able to send messages on the channel"
    );
...
fn test_system_tray_connects_to_state_manager() {
    ...
    assert!(
        result.is_ok(),
        "Should connect to state manager successfully"
    );
}
...
fn test_state_changes_reflected_in_system_tray() {
    ...
    assert!(
        device_state.battery_status.is_some(),
        "Battery status should be updated"
    );
    assert_eq!(
