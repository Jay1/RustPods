        ...
        for charging in [true, false] {
            for size in [1.0, 10.0, 50.0, 80.0, 100.0, 200.0] {
                for animation in [0.0, 0.5, 1.0] {
                    ...
                    let _element =
                        battery_icon::battery_icon_display(Some(level), charging, size, animation);
                }
            }
        }
    }
...
fn prop_window_dimensions_reasonable() {
    use rustpods::ui::window_management::{DEFAULT_WINDOW_HEIGHT, DEFAULT_WINDOW_WIDTH};
...
fn prop_image_dimensions_maintain_aspect() {
    ...
    assert!(
        airpods_aspect > 0.5 && airpods_aspect < 2.0,
        "AirPods aspect ratio unreasonable: {}",
        airpods_aspect
    );
    assert!(
        battery_aspect > 0.5 && battery_aspect < 3.0,
        "Battery aspect ratio unreasonable: {}",
        battery_aspect
    );
}
