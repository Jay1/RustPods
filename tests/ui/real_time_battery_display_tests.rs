...
fn test_real_time_battery_display_update_function() {
    ...
    assert_ne!(
        display.battery_status, initial_status,
    ...
    if let Some(status) = display.battery_status.as_ref() {
        assert_eq!(
            status.battery.left,
            Some(70),
            "Left earbud level should be updated"
        );
        assert_eq!(
            status.battery.charging,
            Some(AirPodsChargingState::LeftCharging),
            "Charging state should be updated"
        );
    } else {
    ...
    assert_eq!(
        display.previous_levels,
        Some((Some(80), Some(75), Some(90))),
        "Previous levels should be set to old values"
    );
}
