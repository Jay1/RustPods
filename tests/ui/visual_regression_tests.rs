...
use rustpods::ui::{
    ...
    theme::{self, Theme},
    window_management::{DEFAULT_WINDOW_HEIGHT, DEFAULT_WINDOW_WIDTH},
...
fn test_window_dimensions_locked() {
    ...
    assert_eq!(
        ...
        "Base window width should not change"
    );
    assert_eq!(
        DEFAULT_WINDOW_HEIGHT, 500,
        "Window height increased to accommodate toast"
    );
...
fn test_battery_icon_dimensions() {
    ...
    let element =
        battery_icon::battery_icon_display(battery_level, false, size, animation_progress);
...
fn test_airpods_image_dimensions() {
    ...
    assert_eq!(
        AIRPODS_WIDTH, 270.0,
        "AirPods width carefully tuned - do not change"
    );
    assert_eq!(
        AIRPODS_HEIGHT, 230.0,
        "AirPods height for proper alignment - do not change"
    );
...
fn test_button_sizes_enhanced() {
    ...
    assert_eq!(
        ENHANCED_BUTTON_SIZE,
        ORIGINAL_BUTTON_SIZE * 1.5,
...
fn test_font_sizes_locked() {
    ...
    assert_eq!(
        BATTERY_TEXT_SIZE, 24.0,
        "Battery text must be 24px for proper visibility"
    );
    assert_eq!(HEADER_TEXT_SIZE, 20.0, "Header text standardized at 20px");
    assert_eq!(
        SEARCH_ICON_SIZE, 48.0,
        "Search icon increased for visibility"
    );
...
fn test_layout_spacing_locked() {
    ...
    assert_eq!(
        IMAGE_TO_BATTERY_GAP, 5.0,
        "Tight 5px gap between image and batteries"
    );
    assert_eq!(BATTERY_TO_TEXT_GAP, 8.0, "Standard 8px battery-to-text gap");
    assert_eq!(
        LEFT_RIGHT_BATTERY_PADDING, 15.0,
        "15px padding between L/R batteries"
    );
...
fn test_svg_generation_stable() {
    ...
    for (level, charging) in test_cases {
        let element = battery_icon::battery_icon_display(level, charging, 80.0, 0.0);
...
fn test_asset_paths_valid() {
    ...
    assert!(
        std::path::Path::new(AIRPODS_IMAGE_PATH).exists(),
...
fn test_layout_bounds_respected() {
    ...
    assert!(
        AIRPODS_COLUMN_WIDTH < WINDOW_WIDTH,
        "AirPods column must fit in window"
    );
    assert!(
        BATTERY_ROW_WIDTH < WINDOW_WIDTH,
        "Battery row must fit in window"
    );
    ...
    assert!(
        ESTIMATED_CONTENT_HEIGHT < WINDOW_HEIGHT,
        "Content must fit in window height"
    );
}
