...
use iced::widget::{button, container, progress_bar, rule, text, text_input};
use iced::{Background, Color};
use rustpods::config;
use rustpods::ui::theme::{
    badge_style, button_style, close_button_style, device_row_style, lavender_button_style,
    secondary_button_style, settings_button_style, settings_icon_color, Theme, BASE, BLUE, GREEN,
    RED, TEXT, YELLOW,
};
...
fn test_all_themes_defined() {
    ...
    for theme in themes {
        ...
        let primary_button =
            <Theme as button::StyleSheet>::active(&theme, &iced::theme::Button::Primary);
        assert!(
            primary_button.background.is_some(),
            "Primary button should have a background"
        );

        let secondary_button =
            <Theme as button::StyleSheet>::active(&theme, &iced::theme::Button::Secondary);
        assert!(
            secondary_button.text_color != Color::TRANSPARENT,
            "Secondary button should have a text color"
        );

        // Test text input style
        let text_input_style =
            <Theme as text_input::StyleSheet>::active(&theme, &iced::theme::TextInput::Default);
        assert!(
        ...
        let container_style = <Theme as container::StyleSheet>::appearance(
            &theme,
        ...
        assert_eq!(
            text_style.color,
            Some(Color::WHITE),
            "Text style should use the provided color"
        );
    }
}
...
fn test_theme_conversion() {
    ...
    assert_eq!(
        config::Theme::from(Theme::CatppuccinMocha),
        config::Theme::System
    ); // CatppuccinMocha maps to System in this implementation
}
...
fn test_theme_button_states() {
    ...
    for theme in themes {
        ...
        assert!(
            active.text_color != Color::TRANSPARENT,
            "Active button should have a text color"
        );
        assert!(
            hovered.text_color != Color::TRANSPARENT,
            "Hovered button should have a text color"
        );
        assert!(
            pressed.text_color != Color::TRANSPARENT,
            "Pressed button should have a text color"
        );
    }
}
...
fn test_text_input_states() {
    ...
    for theme in themes {
        ...
        assert!(
            active.border_color != focused.border_color
                || active.border_width != focused.border_width,
...
fn test_progress_bar_appearance() {
    ...
    for theme in themes {
        ...
        assert!(
            ...
            "Progress bar should have a background"
        );
        assert!(
            appearance.bar != Background::Color(Color::TRANSPARENT),
            "Progress bar should have a bar color"
        );
    }
}
...
fn test_rule_appearance() {
    ...
    for theme in themes {
        ...
        assert!(
            appearance.color != Color::TRANSPARENT,
            "Rule should have a color"
        );
...
fn test_helper_style_functions() {
    ...
    let badge_appearance =
        <Theme as container::StyleSheet>::appearance(&Theme::CatppuccinMocha, &badge);
    assert!(
        badge_appearance.border_radius.ne(&0.0.into()),
    ...
    let row_appearance =
        <Theme as container::StyleSheet>::appearance(&Theme::CatppuccinMocha, &row);
    assert!(
    ...
    let lavender_appearance =
        <Theme as button::StyleSheet>::active(&Theme::CatppuccinMocha, &lavender);
    assert!(
    ...
    let settings_appearance =
        <Theme as button::StyleSheet>::active(&Theme::CatppuccinMocha, &settings);
    assert!(
    ...
    let secondary_appearance =
        <Theme as button::StyleSheet>::active(&Theme::CatppuccinMocha, &secondary);
    assert!(
