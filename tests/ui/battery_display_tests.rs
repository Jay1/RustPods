...
fn create_battery_status(
    left: Option<u8>,
    right: Option<u8>,
    case: Option<u8>,
    charging: AirPodsChargingState,
...
fn test_partial_battery_info() {
    ...
    // Missing left AirPod
    let missing_left =
        create_battery_status(None, Some(80), Some(90), AirPodsChargingState::CaseCharging);
    display.update(Some(missing_left));

    // Missing right AirPod
    let missing_right =
        create_battery_status(Some(75), None, Some(90), AirPodsChargingState::CaseCharging);
    display.update(Some(missing_right));

    // Missing case
    let missing_case =
        create_battery_status(Some(75), Some(80), None, AirPodsChargingState::NotCharging);
    display.update(Some(missing_case));

    // Missing all (should handle gracefully)
    let missing_all = create_battery_status(None, None, None, AirPodsChargingState::NotCharging);
...
fn test_real_time_battery_display_helper_methods() {
    ...
    assert_eq!(
        display_start.get_animated_level(current, previous),
        previous
    );
