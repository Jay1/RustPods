...
use rustpods::airpods::battery_intelligence::{BatteryEventType, BatteryIntelligence};
...
fn validate_prediction_accuracy() {
    ...
    println!(
        "Left battery - Expected: {}, Predicted: {}",
        expected_left, left_prediction
    );
    println!(
        "Right battery - Expected: {}, Predicted: {}",
        expected_right, right_prediction
    );
    ...
    assert!(
        left_error <= 10,
        "Left battery prediction error too high: {}% (target: <=10%)",
        left_error
    );
    assert!(
        right_error <= 10,
        "Right battery prediction error too high: {}% (target: <=10%)",
        right_error
    );
    ...
    assert!(
        (0.3..=0.8).contains(&left_confidence),
        "Left confidence out of expected range: {:.2} (expected: 0.3-0.8)",
        left_confidence
    );
    assert!(
        (0.3..=0.8).contains(&right_confidence),
        "Right confidence out of expected range: {:.2} (expected: 0.3-0.8)",
        right_confidence
    );
}
...
fn validate_charging_cycle_handling() {
    ...
    for _ in 0..6 {
        // 6 updates over 1 hour, 10 minutes apart
    ...
    let charging_events = profile
        .events
        .iter()
        .filter(|e| {
            e.event_type == BatteryEventType::ChargingStarted
                || e.event_type == BatteryEventType::ChargingStopped
        })
        .count();
    ...
    assert!(
        charging_events >= 2,
        "Expected at least 2 charging state change events, got {}",
        charging_events
    );
    ...
    assert!(
        left_estimate.is_real_data,
        "Left earbud should have real data"
    );
    assert!(
        right_estimate.is_real_data,
        "Right earbud should have real data"
    );
    ...
    assert!(
        left_estimate.time_to_next_10_percent.is_some(),
        "Missing time_to_next_10_percent estimate for left earbud after charging cycle"
    );
    assert!(
        right_estimate.time_to_next_10_percent.is_some(),
...
fn validate_asymmetric_battery_drain() {
    ...
    let left_time_to_critical = left_estimate
        .time_to_critical
        .unwrap_or(Duration::from_secs(0));
    let right_time_to_critical = right_estimate
        .time_to_critical
        .unwrap_or(Duration::from_secs(0));
    ...
    assert!(
        left_estimate.level < right_estimate.level,
    ...
    if left_time_to_critical > Duration::from_secs(0)
        && right_time_to_critical > Duration::from_secs(0)
    {
        // Check that the left earbud will reach critical level sooner
        assert!(
            left_time_to_critical < right_time_to_critical,
            "Left earbud should reach critical level sooner due to faster drain rate"
        );
    } else if left_time_to_critical == Duration::from_secs(0)
        && right_time_to_critical == Duration::from_secs(0)
    {
        ...
        assert!(
            left_estimate.level <= 50.0 && right_estimate.level > 50.0,
...
fn validate_partial_data_handling() {
    ...
    intelligence.update_device_battery(
        ...
        Some(85),
        None,
        Some(100), // Right earbud data missing
        false,
        false,
        false,
        true,
        false, // Right not in ear
        Some(-45),
    );
    ...
    assert!(
        left_estimate.level > 0.0,
        "Missing left earbud estimate despite having data"
    );

    // Right should have an estimate based on previous data
    assert!(
        right_estimate.level > 0.0,
        "Missing right earbud estimate despite having previous data"
    );
    ...
    assert!(
        left_confidence > 0.5,
        "Left confidence too low with real data: {}",
        left_confidence
    );

    // Right confidence should be lower but still positive
    assert!(
        right_confidence > 0.0 && right_confidence < left_confidence,
        "Right confidence should be positive but lower than left: {}",
        right_confidence
    );
