...
fn benchmark_storage_efficiency() {
    ...
    assert!(
        profile_path.exists(),
        "Profile file not found at expected path: {:?}",
        profile_path
    );
    ...
    println!(
        "Intelligent storage size: {} bytes",
        intelligent_storage_size
    );
    ...
    if intelligent_storage_size <= raw_storage_size {
        ...
        let reduction_percentage = ((raw_storage_size - intelligent_storage_size) as f64
            / raw_storage_size as f64)
            * 100.0;
        ...
        assert!(
            ...
            "Storage reduction target not met: {:.2}% (target: 90%+)",
...
fn benchmark_battery_prediction_performance() {
    ...
    println!(
        "Average battery estimation time: {:.2} microseconds",
        avg_time_micros
    );
    ...
    assert!(
        avg_time_micros < 1000.0,
        "Battery estimation performance target not met: {:.2} microseconds (target: <1000)",
        avg_time_micros
    );
}
