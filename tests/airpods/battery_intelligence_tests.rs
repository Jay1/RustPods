...
use rustpods::airpods::battery_intelligence::{
    ...
    DepletionTarget, DepletionRateSample, DepletionRateBuffer,
    BatteryEvent
};
...
fn test_device_profile_singleton_behavior() {
    ...
    assert_eq!(intelligence.device_profile.as_ref().unwrap().device_name, "AirPods 1");
    
    // Create second profile - should replace the first one
    intelligence.ensure_device_profile("address2", "AirPods 2");
    assert_eq!(intelligence.device_profile.as_ref().unwrap().device_name, "AirPods 2");
    assert_eq!(intelligence.device_profile.as_ref().unwrap().device_address, "address2");
...
fn test_battery_update_and_significance_filtering() {
    ...
    assert_eq!(intelligence.device_profile.as_ref().unwrap().events.len(), initial_events);
    
    // Update with significant change (battery drop)
    intelligence.update_device_battery(
        "test_address", "Test AirPods",
        Some(80), Some(80), Some(100),
        false, false, false,
        false, false, None
    );
...
fn test_charging_state_detection() {
    ...
    let has_charging_event = profile.events.iter().any(|event| {
        matches!(event.event_type, BatteryEventType::ChargingStarted)
    });
...
fn test_in_ear_state_detection() {
    ...
    assert!(profile.events.len() > initial_events);
    let has_usage_event = profile.events.iter().any(|event| {
        matches!(event.event_type, BatteryEventType::UsageStarted)
    });
...
fn test_event_classification() {
    ...
    intelligence.update_device_battery(
        "test_device", "Test AirPods",
        Some(70), Some(70), Some(90), // 20% drop
        false, false, false,
        true, true,
        Some(-45)
    );
...
fn test_save_and_load() {
    ...
    let original_device_address = intelligence.device_profile.as_ref().unwrap().device_address.clone();
    let original_device_name = intelligence.device_profile.as_ref().unwrap().device_name.clone();
    ...
    assert!(new_intelligence.device_profile.is_some(), "Device profile should be loaded");
...
fn test_purge_profiles() {
    ...
    assert!(intelligence.device_profile.is_none(), "Device profile should be None after purging");
...
fn test_depletion_rate_buffer() {
    ...
    if let Some(median_value) = median {
        assert!((median_value - 2.5).abs() < 0.01, "Median should be approximately 2.5");
    }
    ...
    if let Some(mean_value) = mean {
        assert!((mean_value - 2.5).abs() < 0.01, "Mean should be approximately 2.5");
    }
    
    // Test sample count
    assert!(buffer.get_sample_count(DepletionTarget::LeftEarbud) > 0, "Should have samples for left earbud");
    assert_eq!(buffer.get_sample_count(DepletionTarget::RightEarbud), 0, "Should have no samples for right earbud");
    ...
    assert_eq!(empty_confidence, 0.0, "Empty buffer should have 0 confidence");
}
...
fn test_enhanced_data_collection() {
    ...
    assert!(!intelligence.device_profile.as_ref().unwrap().events.is_empty());
    
    // Simulate 5% battery drop
    intelligence.update_device_battery(
        "test_device", "Test AirPods",
        Some(85), Some(85), Some(80),
        false, false, false,
        true, true,
        Some(-45)
    );
    ...
    assert!(event_count_after_first_drop >= 1, "Should have at least one event after first drop");
    ...
    assert!(event_count_after_second_drop >= event_count_after_first_drop, 
            "Should have at least as many events after second drop as after first drop");
    println!("Events after first drop: {}, Events after second drop: {}", 
             event_count_after_first_drop, event_count_after_second_drop);
    
    // Check that we have depletion rate data
    let profile = intelligence.device_profile.as_ref().unwrap();
    let left_samples = profile.depletion_rates.get_sample_count(DepletionTarget::LeftEarbud);
    let right_samples = profile.depletion_rates.get_sample_count(DepletionTarget::RightEarbud);
    
    // We should have at least one sample for each earbud
    assert!(left_samples > 0, "Should have at least one left earbud depletion rate sample");
    assert!(right_samples > 0, "Should have at least one right earbud depletion rate sample");
}
...
fn test_one_percent_precision_estimates() {
    ...
    assert!(left.level >= 66.0 && left.level <= 68.5, 
            "Expected battery level around 67%, got {}", left.level);
    assert!(right.level >= 66.0 && right.level <= 68.5,
            "Expected battery level around 67%, got {}", right.level);
...
fn test_predict_time_until_critical() {
    ...
    if let Some(time_to_critical) = left.time_to_critical {
        ...
        assert!(minutes > 50.0 && minutes < 150.0, 
                "Time to critical should be roughly 100 minutes (got {:.1})", minutes);
    } else {
        ...
        println!("Note: time_to_critical was None, but battery level is {:.1}%", left.level);
    }
}
...
fn test_device_name_change_and_singleton_behavior() {
    ...
    assert!(intelligence.device_profile.is_some());
    assert_eq!(intelligence.device_profile.as_ref().unwrap().device_name, "Test AirPods");
...
fn test_event_tracking() {
    ...
    // Check initial state
    assert_eq!(intelligence.device_profile.as_ref().unwrap().events.len(), 0);
    ...
    assert_eq!(intelligence.device_profile.as_ref().unwrap().events.len(), 1);
    ...
    assert_eq!(intelligence.device_profile.as_ref().unwrap().events.len(), 2);
