...
use rustpods::airpods::{
    airpods_all_models_filter, airpods_nearby_filter, airpods_pro_filter, AirPodsType,
};
use rustpods::bluetooth::{BleEvent, BleScanner, ScanConfig};
        ...
        while let Some(event) = events.recv().await {
            match event {
                BleEvent::DeviceDiscovered(device) => {
                    ...
                    let _all_airpods = scanner_clone
                        .get_filtered_airpods(&airpods_all_models_filter())
                        .await;
                }
                ...
                BleEvent::ScanCycleCompleted { devices_found } => {
                    ...
                    let _all_airpods = scanner_clone
                        .get_filtered_airpods(&airpods_all_models_filter())
                        .await;
                    let pro_airpods = scanner_clone
                        .get_filtered_airpods(&airpods_pro_filter())
                        .await;
                    let nearby_airpods = scanner_clone
                        .get_filtered_airpods(&airpods_nearby_filter(-70))
                        .await;
