...
use rustpods::bluetooth::{receiver_to_stream, BleEvent, BleScanner, EventFilter, ScanConfig};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tokio::spawn(async move {
        while let Some(event) = airpods_stream.next().await {
            if let BleEvent::AirPodsDetected(airpods) = &event {
                println!(
                    "[AIRPODS] Detected: {:?} with battery L:{}% R:{}% Case:{}%",
                    airpods.device_type,
        ...
        while let Some(event) = control_stream.next().await {
            match &event {
                ...
                BleEvent::ScanCycleCompleted { devices_found } => {
                    println!(
                        "[CONTROL] Scan cycle completed, found {} devices",
                        devices_found
                    );
                }
